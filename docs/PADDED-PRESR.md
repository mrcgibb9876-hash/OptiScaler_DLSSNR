...
Ported from [wilsjo2/OptiScaler-DLSSNR-PreSR-Multipass](https://github.com/wilsjo2/OptiScaler-DLSSNR-PreSR-Multipass)
commit `bab959eb`. The notes below are wilsjo2's, adjusted where this fork differs. This fork adds one
guard: the compact texture is checked against the active size before each copy and rebuilt if it
does not match, independent of the feature-rebuild bookkeeping.
...
interpretation. Ray Reconstruction still always runs NR after upscaling; native Vulkan is unchanged.
...
These tests passed locally in the source fork, and again in this fork on 2026-09-12 with the D3D12
debug layer on. They do not load the NVIDIA NR runtime or a game.
