...
| Key | Opens |
|---|---|
| `Insert` | the DLSS 5 Developer Controls panel -- the same key on every route |
| `Alt+O` | OptiScaler's own overlay, with everything it normally has |
...
exactly. **v1.0.0 and earlier used bare `Home`** for the panel, and later builds `Alt+Home`; RE Engine games still
use `Alt+Home` because REFramework owns `Insert` there. Everything else about OptiScaler — the actual
...
Once installed, launch the game, load a save (OptiScaler doesn't run in menus), press **Alt+O** to
open OptiScaler's overlay, turn **DLSS ON**, then press **Insert** for the DLSS 5 panel. Neural
