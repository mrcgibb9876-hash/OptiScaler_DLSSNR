...
struct Window
{
    ...
    Window(int width, int height)
        : hwnd(CreateWindowExW(0, L"STATIC", L"OptiScaler sizing test", WS_POPUP, 0, 0, width, height, nullptr, nullptr,
...
try
{
    ...
    source.OutputWindow = game.hwnd;
    source.Windowed = TRUE;
    source.BufferCount = 2;
    source.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
    source.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
    source.SampleDesc.Count = 1;
    source.SwapEffect = DXGI_SWAP_EFFECT_DISCARD;
    auto local = source;
    ...
    composition.SampleDesc.Count = 1;
    composition.BufferCount = 2;
    ...
    check(compositor->CreateTargetForHwnd(game.hwnd, TRUE, &target));
    ComPtr<IDCompositionVisual> visual;
    check(compositor->CreateVisual(&visual));
    check(visual->SetContent(chain.Get()));
    check(target->SetRoot(visual.Get()));
    check(compositor->Commit());
    ...
    check(chain->GetDesc1(&resized));
    expect(resized.Width == 800 && resized.Height == 450 && resized.SwapEffect == DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL &&
               resized.Flags == 0,
           "composition resize changed contract");
    std::puts("PASS: client-size classification, descriptor preservation, raw/normalized 0x0 DXGI and composition "
              "create/attach/resize (WARP)");
    return 0;
}
